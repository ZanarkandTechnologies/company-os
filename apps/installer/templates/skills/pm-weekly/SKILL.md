---
{"output":"skills/pm-weekly/SKILL.md","runtime":{"name":"pm-weekly","description":"Turn frozen ${unit} evidence into one JSON result for Weekly reports, grounded memory updates, and next-week carry-forward."},"refs":["organization.grouping","daily.existing_memory","memory.employees","memory.decisions","memory.sops"]}
---
...
- Require supplied run ID, week, next week, evidence kind, expected reporting ${units}, and snapshot coverage/cache facts.
...
- Read every `${memory_current}` and frozen extraction JSON linked by `extraction_refs`.
- Read previous ${report_flow} reports and existing `memory/{employees,sops,issues,decisions}/` entries.
- Use this skill's selected report templates, `../../templates/{person,sop,issue,decision}.md`, and `../pm-daily/templates/${memory_template}`; resolve paths from this skill directory.
...
```json
{
  ...
  "artifacts": [{
    "type": "${unit_key}_report",
    "path": "weeks/<week>/reports/${unit_key}s/${unit_key}--<unit-id>.md",
    "title": "<report title>",
    "frontmatter": {},
    "sections": [{
      "heading": "Summary",
...
| `project_report` | `weeks/<week>/reports/projects/project--<unit-id>.md` | `weekly-report.md` |
...
| `department_report` | `weeks/<week>/reports/departments/department--<unit-id>.md` | `weekly-report.md` |
<!-- endwhen -->
...
| `next_week_${unit_key}_memory` | `${memory_next}` | Daily `${memory_template}` |
| `executive_distribution` | `weeks/<week>/outbound/*.md` | `executive-distribution.md` |

## Workflow

- [ ] **1 — Check the complete frozen set.**
  - Require one readable current-week memory for every expected reporting ${unit}, including ${units} confirmed closed during the interval.
  - Missing, unreadable, or wrong-week expected ${unit}: return `artifacts: []` and blockers; no report or memory promotion.
  - Sparse evidence in a present file is valid input, not a missing ${unit} or failed system.
...
- [ ] **2 — Compose ${unit} artifacts in memory.**
...
<!-- when:organization.grouping=department -->
- [ ] **3 — Roll up in memory.**
  - Company derives only from complete Department artifacts; do not create a second aggregation pipeline.
  - Link every immediate Department report in Company source items or report-source frontmatter.
  - Company Department executive summary has one named entry per Department: result/change, open attention, next priority and source report, including quiet or evidence-limited Departments.
  - Department People progress covers every supplied in-scope person; Company consolidates each person across Departments with scope labels and links.
  - Deduplicate by exact Person ID; preserve different Work states, never average them into ratings.
  - Approval owners and receivers are not producers; preserve exact acceptance scope, receiver identity and controls.
  - Elevate material shared results and interventions; do not repeat accepted outputs as priorities or unblocking work.
  - Keep blocker reasoning in Unblocking work; other sections add distinct implications only.
  - Carry material child limitations upward; sparse evidence alone is not system failure.

<!-- endwhen -->

- [ ] **4 — Consolidate grounded memory.**
  - ${answer__memory_employees}
  - ${answer__memory_decisions}
  - ${answer__memory_sops}
  ...
  - One observation may stay ${unit}-only; recurrence supports a proposed procedure, not automatic adoption.
  ...
  - Routine choices stay ${unit}-local; Weekly finalization never turns proposals into approved decisions.
...
- [ ] **5 — Carry forward and describe distribution.**
  - Compose next-week ${unit} Memory for each still-active ${unit} from unresolved attention only; preserve valid existing next-week content if supplied.
  - Do not create new next-week memory for a confirmed closed/archived ${unit}; report contradictory open obligations as closure attention, without reopening it or overwriting existing next-week content.
...
- [ ] **6 — Check and return JSON.**
  ...
  - Limit the footer to one concise bullet per affected ${unit}: issue → consequence → fix.
