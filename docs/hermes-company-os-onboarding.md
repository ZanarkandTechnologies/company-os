...
to work with an SME's existing tools. It does not replace those tools, ingest a
second company database, or generate one skill per company surface.

```text
onboard(company, current_stack)
  -> .hermes.md + connection_receipt.json + next_action
```
...
- Skills are added only for repeatable procedures that need more than source routing.
...
```mermaid
flowchart LR
    A["Ask for current stack once"] --> B["Detect skill, CLI, or MCP"]
    B --> C["Test status and bounded read"]
    C --> D["Map sources to five surfaces"]
    D --> E["Fill and review .hermes.md"]
    E --> F["Start a fresh Hermes session"]
    F --> G["Run representative lookups"]
```
...
6. Start a new Hermes session from that workspace and test representative
