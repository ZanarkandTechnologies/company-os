...
---
# Weekly operating review — Example Company
...
- Read local memory at `weeks/<week>/project-memory/project--<unit-id>.md` and its extraction references.
...
- Use `UTC` and ISO weeks; never infer an evidence interval from the current clock after freezing it.
- Include completed/archived Projects represented in that interval; preserve their reporting Department and confirmed closure rather than silently removing their results or people.
- Resolve current status for those exact Projects; an unreadable status is a gap, not proof of closure or authority to revive Work.
- Write `weekly/context/<run-id>.json` with the complete inventory,
  input paths/hashes and available per-Project coverage evidence references.
...
- Keep Project → Department → Company reasoning inside the skill; do not repeat it here.

## 3. Verify the JSON handoff

- Read the JSON; check week, Project coverage, artifact type/path pairs, sections and sources.
...
- If the expected Project set is incomplete, require blockers and no artifacts.
...
- Render Project → Department → Company reports in that order, then memory and executive output.
